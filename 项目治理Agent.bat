...
node "%CODEX_JS%" resume --cd "%PROJECT_ROOT%" --sandbox workspace-write --ask-for-approval on-request "%GOVERNANCE_SESSION_ID%"

if errorlevel 1 (
  echo.
  echo 固定治理会话恢复失败，正在创建新的治理会话...
  node "%CODEX_JS%" --cd "%PROJECT_ROOT%" --sandbox workspace-write --ask-for-approval on-request
)
